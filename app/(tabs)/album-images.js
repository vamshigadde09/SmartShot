...
export default function AlbumImagesScreen() {
    ...
    const loadImages = async (reset = false) => {
        ...
        try {
            ...
            // Use native module to filter by bucket (images + videos)
            const media = await NativeModules.ScreenshotModule.getAllMedia();
            const filtered = bucketId ? (media || []).filter((m) => String(m.bucketId) === String(bucketId)) : (media || []);
            const all = filtered.map((m, index) => ({ id: m.id || `m_${index}`, uri: m.uri, filename: m.name || `media_${index}`, creationTime: m.dateAdded || Date.now() / 1000, mediaType: m.mediaType || 'image' }));
    ...
    const SmartMedia = ({ item, style }) => {
        try {
            if (item.mediaType === 'video') {
                ...
                return <VideoLib source={{ uri: item.uri }} style={style} resizeMode="cover" shouldPlay={false} isMuted />;
            }
        } catch { }
                                    ...
                                    <View style={styles.viewerPage}>
                                        {item.mediaType === 'video' ? (() => {
                                            try {
                                                ...
                                                return <SmartImage uri={item.uri} style={styles.viewerImage} mode="contain" />
                                            }
                                        })() : (
